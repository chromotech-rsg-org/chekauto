...
export const ProductCatalog: React.FC = () => {
  ...
  const handleContract = (productTitle: string) => {
    console.log('Contract clicked for:', productTitle);
  };

  const handleShowDetails = (productTitle: string) => {
    console.log('Show details clicked for:', productTitle);
  };
    ...
    <section id="implementations" className="bg-[rgba(233,233,233,1)] flex w-full flex-col items-center pb-[95px] px-20 max-md:max-w-full max-md:px-5">
      <div className="bg-white z-10 flex mt-[-200px] w-full max-w-[1274px] flex-col items-stretch pt-[94px] pb-[166px] px-[71px] rounded-[20px_20px_0px_0px] max-md:max-w-full max-md:pb-[100px] max-md:px-5">
        <nav className="flex items-center gap-[40px_65px] text-sm text-black font-medium text-center leading-none max-md:max-w-full">
          <img
            src="https://api.builder.io/api/v1/image/assets/TEMP/a6b047f6ad1235731d4cd4774eb980879a525c02?placeholderIfAbsent=true"
            alt="CHEKAUTO Logo"
            className="aspect-[5.68] object-contain w-44 shrink-0 max-w-full"
          />
          {categories.map((category) => (
            <button
              key={category}
              onClick={() => setActiveCategory(category)}
              className={`self-stretch my-auto hover:text-[rgba(240,186,29,1)] transition-colors ${
                activeCategory === category ? 'text-[rgba(240,186,29,1)]' : ''
              }`}
            >
              {category}
              {activeCategory === category && (
                <div className="bg-[rgba(240,186,29,1)] flex shrink-0 h-[5px] mt-1" />
              )}
            </button>
          ))}
        </nav>
        ...
        {productSections.map((section, sectionIndex) => (
          <div key={sectionIndex}>
            <div className="z-10 flex items-stretch gap-5 text-sm text-black font-medium text-center leading-loose flex-wrap justify-between mt-[73px] max-md:mt-10">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="border flex flex-col items-stretch justify-center px-[70px] py-5 rounded-[8px_8px_0px_0px] border-[rgba(204,204,204,1)] border-solid max-md:px-5">
                  <div>
                    {section.title.split('|')[0]} |{" "}
                    <span className="underline text-[rgba(250,169,84,1)]">
            ...
            <div className="max-md:max-w-full">
              <div className="gap-5 flex max-md:flex-col max-md:items-stretch">
                {section.products.map((product, productIndex) => (
                  <div key={productIndex} className="w-[33%] max-md:w-full max-md:ml-0">
                    <ProductCard
                      title={product.title}
                      image={product.image}
                      buttonText={product.buttonText}
                      onContract={() => handleContract(product.title)}
                      onShowDetails={() => handleShowDetails(product.title)}
                    />
                  </div>
                ))}
              </div>
        ...
        <div className="border z-10 flex flex-col items-stretch text-sm text-black font-medium text-center leading-loose justify-center mt-[49px] px-[70px] py-5 rounded-[8px_8px_0px_0px] border-[rgba(204,204,204,1)] border-solid max-md:mt-10 max-md:px-5">
          <div>
        ...
        <div className="border flex mb-[-33px] w-[350px] max-w-full flex-col items-center px-[31px] py-[26px] rounded-[0px_0px_8px_8px] border-[rgba(204,204,204,1)] border-solid max-md:mb-2.5 max-md:px-5">
          <h3 className="text-[rgba(22,28,45,1)] text-xl font-semibold leading-none text-center">
            OUTROS IMPLEMENTOS
          </h3>
          <div className="bg-[rgba(240,186,29,1)] self-stretch flex flex-col items-stretch justify-center mt-[42px] px-[35px] py-[54px] max-md:mt-10 max-md:px-5">
            <img
          ...
          <button className="rounded bg-[rgba(240,186,29,1)] self-stretch flex flex-col items-center text-base text-black font-semibold whitespace-nowrap text-center leading-loose mt-5 pt-3.5 pb-[23px] px-[35px] hover:bg-[rgba(220,166,9,1)] transition-colors max-md:px-5">
            CONSULTAR
          </button>
          <button className="text-black text-sm font-medium leading-[25px] text-center underline mt-[17px] hover:text-[rgba(240,186,29,1)] transition-colors">
            Mostrar detalhes
          </button>
        </div>
      </div>
