  ...
  return <section id="implementations" className="bg-[rgba(233,233,233,1)] flex w-full flex-col items-center pb-[95px] px-20 max-md:max-w-full max-md:px-5">
      <div className="bg-white z-10 flex mt-[-200px] w-full max-w-[1274px] flex-col items-stretch pt-[94px] pb-[166px] px-[71px] rounded-[20px_20px_0px_0px] max-md:max-w-full max-md:pb-[100px] max-md:px-5">
        <nav className="flex items-center gap-[40px_65px] text-sm text-black font-medium text-center leading-none max-md:max-w-full">
          ...
          {categories.map(category => <button key={category} onClick={() => setActiveCategory(category)} className={`self-stretch my-auto hover:text-brand-yellow transition-colors ${activeCategory === category ? 'text-brand-yellow' : ''}`}>
              {category}
              {activeCategory === category && <div className="bg-brand-yellow flex shrink-0 h-[5px] mt-1" />}
            </button>)}
        </nav>
              ...
              <div className="gap-5 flex max-md:flex-col max-md:items-stretch">
                {section.products.map((product, productIndex) => <div key={productIndex} className="w-[33%] max-md:w-full max-md:ml-0 flex flex-col h-full">
                  <div className="border flex flex-row items-center justify-between px-5 py-4 rounded-[8px_8px_0px_0px] border-[rgba(204,204,204,1)] border-solid text-sm text-black font-medium">
                    ...
                    <ProductCard id={product.id} title={product.title} image={product.image} buttonText={product.buttonText} />
                  </div>)}
              </div>
            </div>
          </div>)}
        ...
        <div className="flex flex-col items-center mt-[49px] max-md:mt-10">
          <div className="border flex flex-row items-center justify-between px-5 py-4 rounded-[8px_8px_0px_0px] border-[rgba(204,204,204,1)] border-solid text-sm text-black font-medium w-[350px] max-w-full">
            ...
            <button onClick={() => navigate('/produto/outros')} className="text-brand-yellow hover:text-brand-yellow-dark transition-colors">
              Saiba mais
            </button>
          </div>
          <div className="border flex w-[350px] max-w-full flex-col items-center px-[31px] py-[26px] rounded-[0px_0px_8px_8px] border-[rgba(204,204,204,1)] border-solid max-md:px-5">
            ...
            <div className="bg-brand-yellow self-stretch flex flex-col items-stretch justify-center mt-[42px] px-[35px] py-[54px] max-md:mt-10 max-md:px-5">
              <img src="https://api.builder.io/api/v1/image/assets/TEMP/f82672a60befd28412ea9ad539d6988b3d47c203?placeholderIfAbsent=true" alt="Outros implementos" className="aspect-[1.85] object-contain w-[177px]" />
            </div>
            ...
            <button onClick={() => navigate('/produto/outros')} className="rounded bg-brand-yellow self-stretch flex flex-col items-center text-base text-black font-semibold whitespace-nowrap text-center leading-loose mt-5 pt-3.5 pb-[23px] px-[35px] hover:bg-brand-yellow-dark transition-colors max-md:px-5">
              CONSULTAR
            </button>
            <button onClick={() => navigate('/produto/outros')} className="text-black text-sm font-medium leading-[25px] text-center underline mt-[17px] hover:text-brand-yellow transition-colors">
              Mostrar detalhes
            </button>
          </div>
        </div>
